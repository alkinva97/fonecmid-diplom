...
Процедура ВыполнитьАвтозаполнение(ТабЧасть, ДатаНачала, ДатаОкончания) Экспорт
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	СУММА(ЕСТЬNULL(ВКМ_ВыполненыеКлиентуРаботы.КоличествоЧасов, 0)) КАК КоличествоЧасов,
		|	СУММА(ЕСТЬNULL(ВКМ_ВыполненыеКлиентуРаботы.СуммаКОплате, 0)) КАК СуммаКОплате
		|ИЗ
